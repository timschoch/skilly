...
```sh
npx github:timschoch/skilly setup                       # one-time: secrets, .skilly/config.json, caller workflow, setup-project skill
...
2. `npx -y github:timschoch/skilly setup` — switches to `chore/skilly-setup`, sets the App secrets, writes `.skilly/config.json` (no bundles), writes `.github/workflows/skilly-sync.yml`, adds formatter ignores, installs the `setup-project` skill, commits, opens the PR — then launches Claude Code (Sonnet) with `/setup-project`.
