...
export async function add(names, { cwd = process.cwd(), bundlesDir, forcePush = false }) {
  ...
  for (const [source, skills] of skillsBySource) {
    ...
    if (missing.length) throw new Error(`skills add left these uninstalled: ${missing.map((s) => `${source}: ${s}`).join(', ')}`);
  }
  ...
  commit(cwd, `chore(skilly): add ${names.join(', ')}`, { forcePush });
