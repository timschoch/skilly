...
import { resolveBundles, sourceRepo } from './resolve.js';
...
export function pickPrivateOwner(sources, repoOwner, isPrivate) {
  ...
  if (owners.size > 1) {
    throw new Error(
      `private sources from two owners (${[...owners].sort().join(', ')}) — one private-owner per consumer (known v1 limit)`,
    );
  }
  return owners.size ? [...owners][0] : null;
}

function ensurePrivateOwner(cwd, bundlesDir, self) {
  ...
  const { sources } = resolveBundles(skillyFile.read(cwd).bundles, bundlesDir, { selfSource: self });
  const isPrivate = (source) =>
    run('gh', ['api', `repos/${sourceRepo(source)}`, '--jq', '.private'], { cwd }) === 'true';
...
export async function add(names, { cwd = process.cwd(), bundlesDir, forcePush = false, push = true }) {
  ...
  for (const name of names) {
    ...
    if (match.type === 'bundle') {
      ...
      const { sources } = resolveBundles([name], bundlesDir, { selfSource: self });
  ...
  for (const [source, skills] of skillsBySource) {
    ...
    if (missing.length)
      throw new Error(
        `skills add left these uninstalled: ${missing.map((s) => `${source}: ${s}`).join(', ')} — nothing was committed`,
      );
  }
  ...
  if (addedBundles.length) ensurePrivateOwner(cwd, bundlesDir, self);
