import { readFileSync, mkdirSync, readdirSync, renameSync, rmdirSync, lstatSync, symlinkSync } from 'node:fs';
...
export function linkSkillsDir(cwd) {
  ...
  const stat = (() => { try { return lstatSync(claudeDir); } catch { return null; } })();
  if (stat?.isSymbolicLink()) return false;
  if (stat) {
    for (const entry of readdirSync(claudeDir)) renameSync(join(claudeDir, entry), join(agentsDir, entry));
    rmdirSync(claudeDir);
  }
