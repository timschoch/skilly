...
// A name is a bundle if .skilly/config.json or the hub says so (a bundle the hub
// dropped is still removable); a bare skill must be in the lock.
export async function remove(names, { cwd = process.cwd(), bundlesDir, forcePush = false, push = true }) {
  ...
  for (const name of names) {
    if (skillyFile.read(cwd).bundles.includes(name) || isHubBundle(name)) {
      ...
      throw new Error(`no bundle or skill named "${name}" here — check .skilly/config.json and skills-lock.json`);
    }
  }
  ...
  const { sources } = resolveBundles(remaining, bundlesDir, { selfSource: self });
  ...
  commit(cwd, skillyMessage('remove', names), { forcePush, push });
