...
// Biome 2.2 and up reads a bare folder and warns on a trailing /** (useBiomeIgnoreFolder),
// Biome 1.x needs the glob, and .prettierignore reads a folder the way .gitignore does.
const SKILLY_IGNORES = [...SKILLY_FILES, ...SKILLY_DIRS];
const SKILLY_IGNORES_1X = [...SKILLY_FILES, ...SKILLY_DIRS.map((dir) => `${dir}/**`)];
  ...
  if (biomePath) {
    if (biomePath.endsWith('.jsonc')) {
      ...
      if (Array.isArray(config.files.includes)) {
        ...
        config.files.ignore = [...new Set([...(config.files.ignore ?? []), ...SKILLY_IGNORES_1X])];
      }
